...
orderSINR = 10;           % Order of 1st GL quadrature for OP-SINR
...
for km = 1:length(m)
    ...
    for ku = 1:length(U)
        ...
        for kn = 1:length(N )
            ...
            pout_calc_int(kn,km,ku) = CalcOutageFAMA(gam, N(kn), dm, U(ku), 'Integral', order, m(km), famatype,flag2int,flagmf, avg_snr);        
        ...
        for kn = 1:length( Nsimul )
            ...
            pout_calc_gl(kn,km,ku) = CalcOutageFAMA(gam, Nsimul(kn), dm, U(ku), 'Quadrature', order, m(km),famatype,flag2int,flagmf, avg_snr, order);

            % Monte Carlo simulation
            Nsamples = max( round( factor_sim * ( 1 / pout_calc_gl(kn,km,ku) ) ), min_samples );
            pout_sim(kn,km,ku) = SimOutage_BlocksFAMA(Nsamples, gam, U(ku), sqrt(dm), Nsimul(kn), m(km),famatype,flagsimul, avg_snr);
