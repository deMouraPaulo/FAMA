...
% Follow the steps:
% 1) Run the code Fig5_3.m to generate data; 
% 2) Run this code.
% 3) Check the mean and variance analysis in the command window.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ylabel('Outage Probability', 'FontSize', 24, 'Interpreter','latex');
xlim ([5, 20]);
ylim([1e-6, 1])
...
annotation(figure(1),'textbox',...
    ...
    ['$U = $ ', num2str(U), ',  m = ',num2str(m),', $\alpha = $ ',num2str(alphaCF)], ...
    ['$d_0=$ ',num2str(d0), ', $d=$ ',num2str(d) ] },...
    'Interpreter','latex',...
...
% Expressions for mean and varince of Nakagami-RV with parameters (m,omega)
% are given in Proakis, Eq. (2.3-71):
% mean(Nkg) = ( (gamma(m + 0.5) ./ gamma(m)) ) .* sqrt(omega ./ m) ;
% var(Nkg)  = omega .* ( 1 - (1./m) .* ( gamma(m + 0.5) ./ gamma(m) ).^2 );
...
disp('ANALYSIS OF THE NAKAGAMI-M RVs FOR DESIRED AND INTERFERENCE USERS:')
disp(['For number of antennas at BS: ',num2str(antBS)])
disp(['Mean of the desired envelope: ', num2str(mean_des) ])
disp(['Variance of the desired envelope: ', num2str(var_des) ])
disp('---')
disp(['For number of antennas at BS: ',num2str(antBS(2)),' / ',num2str(antBS(1))])
