...
for ku = 1:length(U)
    ...
    %-----------------------------------------------------------------
        for kg = 1:length(gam)
            ...
            pout_calc_int(kw,kg,ku) = CalcOutageFAMA(gam(kg), N, dm, U(ku), 'Integral', order, m, famatype,flag2int,flagmf, avg_snr);

            % Gauss-Laguerre Quadrature calculation
            pout_calc_gl(kw,kg,ku) = CalcOutageFAMA(gam(kg), N, dm, U(ku), 'Quadrature', order, m,famatype,flag2int,flagmf, avg_snr, order);

            % Monte Carlo simulation
            Nsamples = max( round( factor_sim * ( 1 / pout_calc_gl(kw,kg,ku) ) ), min_samples );
            pout_sim(kw,kg,ku) = SimOutage_BlocksFAMA(Nsamples, gam(kg), U(ku), sqrt(dm), N, m,famatype,flagsimul, avg_snr);
