...
for kn = 1:length(N)
    ...
    for ku = 1:length(U)
        ...
        % OP-based SIR
        pout_calc_sir(kn,ku,:) = CalcOutageFAMA(gam, N(kn), dm, U(ku), 'Quadrature', order, m,famatype,famatype,flag2int, Inf, orderSINR);
        ...
        for kg = 1:length(avg_snr )
            ...
            pout_calc_int(kn,ku,kg) = CalcOutageFAMA(gam, N(kn), dm, U(ku), 'Integral', order, m,famatype,flag2int,flagmf, avg_snr(kg), orderSINR);
        ...
        for kg = 1:length(avg_snr_sim )
            ...
            pout_calc_gl(kn,ku,kg) = CalcOutageFAMA(gam, N(kn), dm, U(ku), 'Quadrature', order, m,famatype,flag2int,flagmf, avg_snr_sim(kg), orderSINR);
            
            % Monte Carlo simulation
            NsamplesAdjusted = max( round( 100 * ( 1 / pout_calc_gl(kn,ku,kg) ) ), Nsamples );
            pout_sim(kn,ku,kg) = SimOutage_BlocksFAMA(NsamplesAdjusted,gam, U(ku), sqrt(dm), N(kn), m,famatype,flagsimul, avg_snr_sim(kg));
